...
static void app_calculator(void) {
	bootscreen_show(4);   /* show app‑specific bootscreen */
    ui_title(33, 62, green bold, "Calculator placeholder");
    pal_pause();
}
...
static void cmd_ver(void) {
    ui_title(33, 62, RED bold, "VER: 6.48.21");
    pal_pause();
}
...
static void cmd_unst_all(void) {
    uninstall_all(1); //1 to show status message
    //ui_status(STATUS_SUCCESS);
}
...
static void cmd_install_guest(void) {
    if (user_exists()) {
        ...
        installer_prompt();   /* run full installer (app_id 1 = generic) */
    }
}
...
static const cmd_entry_t cmd_table[] = {
    ...
    { "install",         cmd_install_guest,   1, 0, NULL },   /* guest‑aware install */
...
void prompt(void) {
    ...
    while (1) {
        ...
        ui_init();
        ui_title(10, 60, RED bold underline, "Operating Environment ver: 6.48.21");
        ...
        /* ----- Handle install/uninstall patterns (prefix detection) ----- */
        ...
        /* Loop continues – ui_status already includes a pause */
    }
}