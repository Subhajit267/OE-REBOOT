...
void calculator(void) {
    ...
    while (1) {
        ...
        ui_init();
		int row = 10, col = 75;
        ui_title(row+=3, col, red bold underline, "OE SYSTEM CALCULATOR");
        ui_title(row+=2, col, green bold, "Supported: + - * / // % ^ () [] {}");
        ui_title(row+=1, col, blue bold, "Type E to return");

        ui_title(row+=2, col, yellow bold, "Enter Expression(max length 510): " reset);
        ...
        ui_title(row+=2, col, cyan bold, "Result: " reset);
